...
<script>
// Single shared key across ALL reports — responded state is cumulative and permanent.
// Keyed by post URL (stable identifier), not card index (changes per run).
var STORE_KEY = 'li_connect_responded';
var responded = JSON.parse(localStorage.getItem(STORE_KEY) || '{{}}');
var recFilter = 'all';
var searchVal = '';

// Build index: url -> card index for this report
var urlToIdx = {{}};
document.querySelectorAll('.card[data-url]').forEach(function(card) {{
  var url = card.getAttribute('data-url');
  var idx = card.id.replace('card', '');
  if (url) urlToIdx[url] = idx;
}});

// Restore responded state from shared store on page load
document.querySelectorAll('.card[data-url]').forEach(function(card) {{
  var url = card.getAttribute('data-url');
  if (url && responded[url]) {{
    var idx = card.id.replace('card', '');
    setRespondedUI(idx, true);
  }}
}});
